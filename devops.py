...
def main(unused_args):
    ...
    exist_blue = run_shell('sudo', 'docker-compose', '-p', SERVICE_NAME + '-blue', '-f', fileName + '.blue.yml', 'ps', '|', 'grep', 'Up')
    
    print()
    exist_green = run_shell('sudo', 'docker-compose', '-p', SERVICE_NAME + '-green', '-f', fileName + '.green.yml', 'ps', '|', 'grep', 'Up')
    if exist_blue != '':
